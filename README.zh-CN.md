...
- `low / auto / max` 只控制资源调度，不偷偷降低画质；`auto` 会在人回来使用电脑时自动暂停、空闲时自动满速继续。
...
48GB M5 Pro 默认推荐 `auto`。它会使用 h3.c 的 `--ssd-streaming` 控制统一内存峰值；检测到键鼠活动、明显的其他 CPU 任务或电池供电时暂停 H3，接电且连续空闲 60 秒后自动继续并解除后台策略。暂停期间不占用 CPU/GPU，但推理状态仍保留在内存中；同一权重和参数下不会主动降低画质。
...
| auto | <64 GiB 自动 streaming；人在使用时暂停，接电且空闲 60 秒后自动满速继续 | 否 |
...
未来可以支持量化，但会作为明确的引擎档位加入，而不会把它伪装成无损加速。当前锁定版 h3.c 的实验性 INT8 路径不能和 SSD streaming 同时使用，收益也不足以成为 48GB 机器的默认方案；h3.c 新版正在扩展原生 INT8 Metal 路径。项目会在 48GB M5 Pro 上完成相同提示词、种子、分辨率、NFE、内存和画质对照后再开放该选项。
